  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--foreground)]">My Submissions</h1>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <h3 className="font-semibold text-[var(--foreground)] leading-snug line-clamp-2">{m.title}</h3>
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <span className="text-xs text-[var(--muted)]">{m.journal.title}</span>
                        <span className="text-xs text-[var(--muted)]">Updated {timeAgo(m.updatedAt)}</span>
                      </div>
                    </div>
                    <Link href={`/dashboard/manuscripts/${m.id}`}>
                      <Button size="sm" variant={isRevision ? "default" : "outline"} className="gap-1 flex-shrink-0">
                        {isRevision ? "Upload Revision" : "View"}
